...
<div class="protocolBox">
    <form id="newProtocolForm" class="protocolForm" action="cadastrarProtocolo.php" method="post" >
        ...
        <br/>
        <label class="name">Nome completo*:<input id="name" name="name" type="text" placeholder="Informe seu nome completo..." required ></label>
        <br/>
        ...
        <br/>
        <label class="phone">Telefone para contato*:<input id="tel" name="tel" type="tel" size="11" placeholder="Informe seu telefone no formato 519xxxxxxxx (sem pontos, espaços ou traços)" required ></label>
        <br/>
...
//    $form_tel = validate_phone(str_replace(" ", "", $form_tel));
    ...
    if (!empty($form_problemType) && !empty($form_description) && !empty($form_name) && !empty($form_cpf) && !empty($form_tel) && !empty($form_city) && !empty($form_address)) {
        ...
        echo "<script>alert('Erro ao cadastrar protocolo! Valores faltando ou incorretos!');</script>";
    }
